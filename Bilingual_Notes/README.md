# Notes Hub | 讲道与团契笔记中心

> Sermon and fellowship notes, arranged by teaching date.
> 按实际讲道／团契日期整理的笔记。

## Start Here | 从这里开始

- [Chronological Notes Index | 按时间顺序的笔记索引](./INDEX_Chronological.md)
- [Teaching Notes Summary | 全部 Teaching 总结](./All_Notes_Summary_Chronological.md)
- [Delighting in the Trinity Discipleship Guide | 《喜乐于三一神》门徒训练指南](./2026-01-21_to_02-25_Delighting_in_the_Trinity_Discipleship_Guide.md)
- [Supplemental Study: The Spirit Beautifies | 补充学习：圣灵使人得荣美](./Supplemental_Delighting_in_the_Trinity_Chapter_4_The_Spirit_Beautifies.md)

## Reading Status | 阅读状态

Completed notes use bilingual titles and may contain English, Chinese, or both depending on the original teaching and the note’s current editing stage. The index records the source date and whether an item is a completed note or a source transcript awaiting refinement.

已完成笔记使用双语标题；正文会按原讲道语言和当前整理阶段采用英文、中文或双语。索引会标注实际日期，并区分“已完成笔记”与“待精修来源记录”。

## Contribution Rule | 整理规则

Keep the original teaching date, use a descriptive bilingual filename when creating a new polished note, and link it from the chronological index. Preserve direct quotations and clearly distinguish them from summaries or personal reflections.

新建精修笔记时，保留原讲道日期，使用描述性的双语文件名，并加入时间索引。直接引语、内容总结和个人反思必须明确区分。
